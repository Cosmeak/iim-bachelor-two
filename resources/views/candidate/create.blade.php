...
@section('content')
    <section class=" mt-28 text-xl flex items-center flex-col">
        ...
        <form class="flex flex-col xl:w-full  xl:px-64 justify-center items-center" method="POST"
            action="{{ route('candidate.store') }}">
            @csrf
            <input type="hidden" name="user_id" value="{{ auth()->user()->id }}">
            <div id="content_inscription_1" class="flex lg:flex-row flex-col justify-center items-center xl:h-xl">
                <div class="flex flex-wrap justify-center">
                    ...
                    <div class="flex flex-col items-start mx-16 my-4">
                        ...
                        <input class="btn-primary text-gray-400" value="{{ old('birth_date') }}" type="date" placeholder="Date de Naissance"
                            name="birth_date">
                        @error('birth_date')
                            <p class="text-red-500 mt-2">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="flex flex-col items-start mx-16 my-4">
                        ...
                        <input class="btn-primary" type="tel" placeholder="Numéro" name="phone_number" value="{{ old('phone_number') }}">
                        @error('phone_number')
                            <p class="text-red-500 mt-2">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div id="content_inscription_2" class="xl:flex-row flex-col justify-center items-center hidden bg">
                <div class="flex flex-wrap justify-center xl:justify-start">
                    <div class="flex flex-col items-start mx-16 my-4">
                        ...
                        <input class="btn-primary" type="url" placeholder="Site web" name="website" value="{{ old('website') }}">
                        @error('website')
                            <p class="text-red-500 mt-2">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="flex flex-col items-start mx-16 my-4">
                        ...
                        <input class="btn-primary" type="text" placeholder="LinkedIn" name="linkedin" value="{{ old('linkedin') }}">
                        @error('linkedin')
                            <p class="text-red-500 mt-2">{{ $message }}</p>
                        @enderror
                    </div>
                    <div id="container_ss" class=" mx-16 mt-4">
                ...
                <div>
                    <div class="flex flex-col items-start">
                        ...
                        <input type="file" class="dashed-hover max-w-sm xl:max-w-2xl rounded-xl py-24 px-8 border-2 border-slate-600 border-dashed bg-white " name="cv" value="{{ old('cv') }}">
                        @error('cv')
                            <p class="text-red-500 mt-2">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div id="submit_inscription" class="hidden">
