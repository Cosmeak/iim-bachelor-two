<header class="w-full h-[100] bg-white hidden xl:flex items-center justify-between px-10">
  ...
  @auth
    <ul class="flex justify-evenly items-center">
      ...
      <form method="POST" action="{{ route('user.logout') }}" class="w-sm px-6 py-4 bg-primary text-white rounded-2xl shadow-md hover:bg-blue-500 transition duration-150 ease-out hover:ease-in focus:outline-none focus:ring-2 cursor-pointer text-center">@csrf<button type="submit" >Se déconnecter</button></form>
...
<header class="flex xl:hidden w-full h-[80] bg-white fixed z-10 shadow-md">
  <div class="flex justify-evenly items-center">
    <a href="/" class="w-fit ml-4"><img src="../img/logo.png" alt="logo" class="h-full w-fit"></a>
